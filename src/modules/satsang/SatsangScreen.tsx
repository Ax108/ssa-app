...
export const SatsangScreen: React.FC = () => {
  ...
  const bhajanListId = yt.bhajan?.includes("list=")
    ? yt.bhajan.split("list=")[1]
    : null;
  const bhajanUrl = bhajanListId
    ? youtubePlaylistUrl(CONSTS.ytPList, bhajanListId)
    : null;
  const channelUrl =
  ...
  return (
    <ScreenScroll>
      ...
      <View style={styles.card}>
        ...
        {bhajanUrl ? (
          <Pressable
            style={[styles.podcastTeaser, styles.bhajanTeaser]}
            onPress={() => void openExternalUrl(bhajanUrl)}
            accessibilityRole="link"
            accessibilityLabel={headers.listenSP}
          >
            <Ionicons name="musical-notes" size={40} color={palette.white} />
            <CustomText freeman customStyle={styles.podcastLabel}>
              {headers.listenSP}
            </CustomText>
            <View style={styles.playBtnSm}>
